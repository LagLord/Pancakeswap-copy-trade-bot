...
def handle_event(event):
    try:
        ...
        if to == router and transaction['from'] == web3.toChecksumAddress(address_to_track):
            # print the transaction and its details
            print(transaction)
            data = contract.decode_function_input(transaction['input'])
            print(data)
            if 'swap' in str(data[0]):
                check_success = check_holders_1k(data[1]['path'][0])
                if check_success:
                    swap_tokenv2(transaction, data)
...
# block_filter = web3.eth.filter({'fromBlock': 'latest', 'address': poolAdd})
# while 1:
...
contract = web3.eth.contract(address=web3.toChecksumAddress(pancakeswap_address.upper()), abi=panabi)
print(type(contract.functions.swapExactTokensForTokensSupportingFeeOnTransferTokens))
...
def swap_tokenv2(receipt, data):
    data, trx, swap_function = data[1], dict(receipt), data[0]
    if "swap" in str(swap_function):
        ...
        if 'amountIn' not in data:
            ...
            data['amountIn'] = int(trx['value']) if int(trx['value']) <= max_bnb else max_bnb
